        ...
        bars = [
          {
            ...
            extraConfig =
              ...
              "status_command while echo \"BCK: $(brightnessctl i -m | cut -d ',' -f4) VOL: $(if pactl get-sink-mute @DEFAULT_SINK@ | grep -q 'yes'; then echo 0%; else pactl get-sink-volume @DEFAULT_SINK@ | awk '{print $5}'; fi) BAT: $(upower -i /org/freedesktop/UPower/devices/battery_BAT0 | grep 'percentage' | awk '{print $2}')$(if upower -i /org/freedesktop/UPower/devices/battery_BAT0 | awk 'FNR == 11 {print $2}' | grep -q 'discharging'; then echo '-'; else echo '+'; fi) $(date +'%Y-%m-%d %l:%M %p')\"; do sleep 1; done;";
          }
        ];
